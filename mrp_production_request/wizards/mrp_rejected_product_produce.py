...
class MrpProductProduce(models.TransientModel):
    _name = "mrp.rejected.product.produce"
    ...
    raw_workorder_line_ids = fields.One2many('mrp.rejected.product.produce.line',
        'raw_product_produce_id', string='Components')
    finished_workorder_line_ids = fields.One2many('mrp.rejected.product.produce.line',
    ...
    def _compute_pending_production(self):
        ...
        # ], limit=1).id
        if self.production_id.product_id.product_rejected:
            self.product_id = self.production_id.product_id.product_rejected
        else:
           product_new = self.env['product.product'].create({
                'name': self.product_id.name+"-rejected",
                'type': 'product',
                'weight': 0,
                'uom_id': self.product_id.uom_id.id,
            })
           self.product_id.product_rejected=product_new.id
    ...
    def action_generate_serial(self):
        ...
        return {
            ...
            'view_mode': 'form',
            'res_model': 'mrp.rejected.product.produce',
            'res_id': self.id,
...
class MrpProductProduceLine(models.TransientModel):
    _name = 'mrp.rejected.product.produce.line'
    _inherit = ["mrp.abstract.workorder.line"]
    _description = "Record production line"

    raw_product_produce_id = fields.Many2one('mrp.rejected.product.produce', 'Component in Produce wizard')
    finished_product_produce_id = fields.Many2one('mrp.rejected.product.produce', 'Finished Product in Produce wizard')
