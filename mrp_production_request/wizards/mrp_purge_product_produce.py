...
class MrpProductProduce(models.TransientModel):
    ...
    totalforone=fields.Integer("Total for one",default=1)
    total_purge = fields.Integer("Total purge", default=0)
    total_unaccounted = fields.Integer("Total unaccounted", default=0)
    ...
    raw_workorder_line_ids = fields.One2many('mrp.purge.product.produce.line',
        'raw_product_produce_id', string='Components')
    finished_workorder_line_ids = fields.One2many('mrp.purge.product.produce.line',
        'finished_product_produce_id', string='By-products')
    production_id = fields.Many2one('mrp.production', 'Manufacturing Order',
        required=True, ondelete='cascade')
    #
    @api.onchange('total_purge')
    def calc_total_purge(self):
        totalpurge=self.total_purge
        totalonlist=0
        move_raw_ids = self.move_raw_ids._origin.filtered(lambda move: move.state not in ('done', 'cancel'))
        remaining_qty = self._get_todo(self.production_id)
        for x in move_raw_ids:
            totalonlist=totalonlist+x.product_qty
        if remaining_qty:
            totalforeach=totalonlist/remaining_qty
            if totalforeach:
                tobeproduced = totalpurge/totalforeach
                if tobeproduced:
                    self.qty_producing=totalpurge/totalforeach
                    self.total_unaccounted=totalpurge % totalforeach
    ...
    def _compute_pending_production(self):
        ...
        # ], limit=1).id
        if self.production_id.product_id.product_purge:
            self.product_id = self.production_id.product_id.product_purge
        else:
           product_new = self.env['product.product'].create({
                'name': self.product_id.name+"-purge",
                'type': 'product',
                'weight': 0,
                'uom_id': self.product_id.uom_id.id,
            })
           self.product_id.product_purge=product_new.id
    ...
    def action_generate_serial(self):
        ...
        return {
            ...
            'view_mode': 'form',
            'res_model': 'mrp.purge.product.produce',
            'res_id': self.id,
...
class MrpProductProduceLine(models.TransientModel):
    _name = 'mrp.purge.product.produce.line'
    _inherit = ["mrp.abstract.workorder.line"]
    _description = "Record production line"

    raw_product_produce_id = fields.Many2one('mrp.purge.product.produce', 'Component in Produce wizard')
    finished_product_produce_id = fields.Many2one('mrp.purge.product.produce', 'Finished Product in Produce wizard')
